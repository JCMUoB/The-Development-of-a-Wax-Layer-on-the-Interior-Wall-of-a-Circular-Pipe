...
epsb        = [0.1,0.5,1.0]; %values of eps 
kb          = [2,10,20]; %values of k
optionb     = [1,2,3,4,5,6,7;3,3,3,3,3,3,3;3,2,1,0,-1,-2,-3]; 
                %Diffusivity function parameters
...
data    = zeros(size(kb,2)*size(epsb,2)*size(optionb,2),16);
    ...
    for epsn=1:size(epsb,2)
        ...
        for optionsn=1:size(optionb,2)
            ...
            data(counter,:) = [k,eps,optionb(1,optionsn),option,D0,h0, ...
                                round(double(hs),3),h(Nt),t0,tvconv,thconv, ... 
                                errv(qvconv),errh(qhconv),elapsedtime, ...
                                Nx,relerr];               
            ...
            filename        = strcat('t_h_v_eps=',num2str(eps), ...
                                ...
                                '(u)=1+(',num2str(optionb(3,optionsn)), ...
                                ')u(1-u).csv');
            ...
            clear filename elapsedtime relerr;
...
OutputData = array2table(double(data),'VariableNames',{'k','eps', ...
                ...
                't_0','tvconv','thconv','||v(tvconv)-vs||_\infty', ...
                '|h(thconv)-h_s|','Elapsed_time','Nx','rel err h'});
